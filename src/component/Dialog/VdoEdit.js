...
export default function Insert(props) {
    ...
    const Submit = async () => {
        if (!vdoAtom.vdoName || !vdoAtom.description || !vdoAtom.src) {
            ...
            console.log(vdoAtom)
            const result = await axios.post(server.UPDATE_VDO, vdoAtom, {
                headers: { "auth-token": localStorage.getItem(ACCESS_TOKEN) },
    ...
    const showModel = () => {
     ...
     const close = async () => {
         ...
         hide()
         setVdoAtom({
            ...vdoAtom, vdoName: undefined,description: undefined,src: undefined,detail: undefined,
          });
     }
                    ...
                    X
                        ...
                        <Form className="insert-box-input">
                            <Form.Group controlId="exampleForm.ControlInput1" className="box-item-insert">
                                ...
                                <Form.Control type="text" name="vdoName" defaultValue={vdoAtom.vdoName} disabled={state.loading} onChange={e => changeHandler(e) }/>
                            </Form.Group>
                            <Form.Group controlId="exampleForm.ControlInput1" className="box-item-insert">
                                <Form.Label>รายละเอียดสั้นๆ</Form.Label>
                                <Form.Control type="text" name="description" defaultValue={vdoAtom.description} disabled={state.loading} onChange={e => changeHandler(e) }/>
                            </Form.Group>
                            <Form.Group controlId="exampleForm.ControlInput1" className="box-item-insert">
                                <Form.Label>Link YouTube</Form.Label>
                                <Form.Control type="text" name="src" defaultValue={vdoAtom.src} disabled={state.loading} onChange={e => changeHandler(e) }/>
                            </Form.Group>
                            ...
                            <Form.Group controlId="exampleForm.ControlTextarea1" className="form-label-vdo">
                                ...
                                <Form.Control as="textarea" name="detail" defaultValue={vdoAtom.detail} disabled={state.loading} rows={3} onChange={e => changeHandler(e) }/>
                            </Form.Group>
